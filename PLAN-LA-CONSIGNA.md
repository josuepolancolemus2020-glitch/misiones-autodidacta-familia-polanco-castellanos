# 📜 La Consigna — el plan entero, con el que se construyó
...
no hubo ninguna avería). El núcleo, la pantalla, el cableado, la sonda, la
revisión y la documentación están hechos y comprobados.
...
| **El núcleo y su prueba** | `js/tools/consigna.js` (el núcleo) y `_dev/test-consigna-node.js` | ✅ El vocabulario, los 17 moldes, el armado, el lector, el repaso, la poda, la fusión y la nube. Nació en el commit `579cf84` con 648 comprobaciones; después de la revisión, **RESULTADO: APRUEBA, 651**, con el armado comparado carácter por carácter con el apartado 7 de este archivo. |
| **La pantalla** | `js/tools/consigna.js` (detrás del cartel «PANTALLA») y `css/consigna.css` | ✅ El anaquel, el compositor y las tres hojas (ver, usar y pegar). Sin un solo `var(--card`. |
...
| **🧸 El cuento para Gemini Storybook** | `js/tools/consigna.js` (el molde `cuento`, sus ocho bloques nuevos del vocabulario y la máquina `storybook`), `css/consigna.css` (el renglón 🧸 de ＋ Nueva) | ✅ Entró el 26 de septiembre de 2026, pedido por el autor: prompts para Gemini Storybook con todos los elementos del storytelling. El molde 18, **Cuento que enseña** (§3.1), con doce bloques —los cinco obligatorios son los cinco que el autor nombró: tema, para quién, lo que se aprende, lo que debe sentir el lector y extensión—, y la séptima máquina, **Storybook** (§4), que es la de un solo molde y no se apunta como la última. Sin una línea de SQL. Una revisión adversarial, antes de publicarlo, cazó que sus rótulos partían y rebautizaban prompts que no son cuentos: arreglado (fuera de un cuento, Pegar lee como antes), con la cabecera en las tres formas, la máquina de antes devuelta al salir del cuento y el repaso afinado. La prueba de Node, **APRUEBA, 775**; la sonda, **APRUEBA, 458**, con la 24 para el cuento. `CACHE_NAME` en `faro-app-v132`. Es la regla 36 de la normativa. |
...
por la tabla. El 26 de septiembre entró el 🧸 cuento para Gemini Storybook
(`faro-app-v132`), tampoco con SQL: `molde` y `maquina` son texto libre. Lo
que se cambie de aquí en adelante sigue la normativa de `CLAUDE.md` y pasa
antes por la sonda.

### Lo que cambió sobre la especificación de abajo

- **El 🧸 cuento para Gemini Storybook entró el 26 de septiembre de 2026**,
  y no estaba en el diseño de partida: el molde **Cuento que enseña** (§3.1)
  y la máquina **Storybook** (§4), con su renglón en ＋ Nueva, su pregunta
  en «¿Cuál elijo?», su propuesta en Pegar y su ejemplo en el §7. Sus
  frases hechas salen de la materia de Storytelling de la casa
  (`COMPENDIO-STORYTELLING.md`): la ficha de hilo, querer contra necesitar,
  la prueba del conector, la emoción sin chantaje y la ética de la historia
  de verdad. **Y antes de publicarlo pasó una revisión adversarial** que
  cazó lo más caro en Pegar: sus rótulos, sueltos, partían y rebautizaban
  prompts que no son cuentos. Desde entonces solo ascienden dentro de un
  cuento, y un texto que no se propone como cuento se lee exactamente como
  antes (§5); la cabecera va en las tres formas (§7); cambiar de molde
  devuelve la máquina de antes y no toca una elegida a mano (§4); y el
  repaso afinó sus tres detectores (§3.1).
...
La especificación de debajo se queda **como referencia**, no como lista de
tareas: es el diseño que se implementó. ⚠️ **Y no se retoca a la ligera:**
la prueba de Node saca de aquí los ejemplos del §7 y los textos de los
moldes y del vocabulario, y compara con ellos lo que arma
`js/tools/consigna.js`; cambiar una frase aquí sin cambiarla allí la hace
suspender, que es exactamente lo que tiene que pasar. **Las reglas, con lo
que se aprendió al construirla, viven en `CLAUDE.md`** («Normativa: las
consignas a las máquinas se escriben en La Consigna»), y eso es lo que se
lee antes de tocar la herramienta. Y como `consigna.js` está cableado, un
error de sintaxis en él tira la casa entera: antes de publicar, la prueba
de Node, la sonda y el bucle de nombres repetidos.
...
- **Molde:** un formato con nombre. En `CSG_MOLDES[id]`: `{ clase, nombre, cuando, recomendado?, bloques: [ {id, ob, rotulo?, para_que?, ejemplo?, frases?, inicial?, plantilla?} ], forma?, sinEncabezados?, rotuloMayus?, cabecera?, maquina?, reordena?, auto?, pseudo?, cierre?, orquestar?, vozPrestada? }`. Una entrada de `bloques` nombra un id del vocabulario y dice si es obligatorio (`ob`); **lo que traiga además (rótulo, para qué, ejemplo, frases, texto inicial, plantilla) SOBRESCRIBE al vocabulario solo dentro de ese molde**. Eso es lo que permite que `formato` se llame «Respuesta» en CO-STAR, `tope` «Rondas» en Careo, `nodos` «Especialistas» en Coordinador o que `ejemplos` traiga frases distintas en cada molde sin tener dos ids. `csgBloqueDef(moldeId, id)` devuelve la mezcla (`Object.assign({}, CSG_BLOQUES[id] || {rotulo: id}, sobreescritura)`), y es lo ÚNICO que consultan el compositor, el armado y el repaso.
...
- **Equivalencias (`CSG_EQUIVALE`):** para «Duplicar en otro molde» cuando el destino no tiene el id: `rol→[identidad] · identidad→[rol] · tarea→[mision, paso, pregunta, borrador, texto] · mision→[tarea] · paso→[tarea] · pregunta→[tarea] · borrador→[tarea] · texto→[tarea] · reglas→[siempre, limites] · siempre→[reglas] · nunca→[reglas] · limites→[reglas] · objetivo→[meta, tarea, leccion] · meta→[objetivo] · leccion→[objetivo, meta] · responde→[formato] · salida→[formato, fin] · fin→[salida] · estilo→[tono] · verificacion→[comprobacion] · comprobacion→[verificacion] · extension→[genero, limites] · genero→[extension]`. Se toma el primer candidato que el destino tenga y esté vacío; dos orígenes al mismo destino se pegan con una línea en blanco; lo demás, bloque libre marcado «traído de «X»: colócalo o quítalo». `comprobacion` es UN solo id en todas las clases (en la v1 había `comprobacion` y `comprobaciones`, y graduar de Encargo a SKILL.md dejaba la comprobación como bloque libre).
...
Abre con **Rápido**. Moldes: Rápido · Encargo completo · CO-STAR · Con ejemplos · Razonado · Investigación con fuentes · Voz prestada · Cuento que enseña · Libre.
...
**Cuento que enseña** (`cuento`, `maquina: 'storybook'`, `cabecera: 'Crea un cuento ilustrado con estas indicaciones.'`) — un cuento ilustrado de unas diez páginas para Gemini Storybook, que emocione y que explique un tema. Pedido por el autor el 26 de septiembre de 2026: «hay que saber el tema de la historia, la extensión de la misma, las sensaciones que se buscan que el lector tenga, la edad del público al que podría ir dirigido, el aprendizaje o lección que se pretende… para poder hacer historias verdaderas que conmuevan, que ayuden al aprendizaje de una temática que, si un contenido es complejo, a través de un breve cuento se explique». Esos cinco son los cinco obligatorios; los otros siete son el oficio de contar: quién vive la historia, dónde pasa, qué se explica con sus datos exactos, con qué voz se cuenta, cómo termina, con qué dibujos y qué no se negocia. Nace con la máquina Storybook puesta (§4) y con ella se arma en `seguida`, con la cabecera delante (§7); con otra máquina, en la forma de esa máquina: el molde no fuerza ninguna, porque el mismo cuento se le puede pedir a Claude como texto para leerlo en voz alta.
- `tema` · Tema de la historia · ● (sobrescribe rótulo, para qué, ejemplo y frases) — De qué trata el cuento y lo que tiene que pasar sí o sí. Si explica algo difícil, dilo aquí con una frase; los datos van en «Lo que el cuento explica». — Ej.: «Un cuento que explique por qué llueve, contado por una gota de agua que tiene miedo de caer.» — Frases: «Un cuento que explique {{tema}} a través de una aventura.» · «Un cuento para acompañar a quien está viviendo {{situacion}}.» · «Basado en algo que pasó de verdad: {{lo_que_paso}}.» · «Tiene que pasar esto: {{hecho}}.» · «Un cuento para contar antes de dormir sobre {{tema}}.»
- `audiencia` · Para quién · ● (sobrescribe rótulo, para qué, ejemplo y frases) — La edad manda en todo lo demás: cuántas palabras lleva cada página, qué palabras se entienden, cuánto miedo se aguanta y qué comparación sirve. — Ej.: «Niños de 6 a 8 años que empiezan a leer solos; muchas veces se lo leerá un adulto en voz alta.» — Frases: «Niños de 3 a 5 años: se lo lee un adulto en voz alta.» · «Niños de 6 a 8 años que empiezan a leer solos.» · «Niños de 9 a 12 años que ya leen bien.» · «Adolescentes que creen que el tema no les interesa.» · «Adultos que nunca entendieron el tema.» · «Un niño de {{edad}} años que se llama {{nombre}}.»
- `personaje` · Protagonista · ○ — Quién vive la historia, qué quiere y qué se lo impide: sin un deseo y algo que lo estorbe no hay cuento, hay una lección con dibujos. Di también cómo es por fuera, para que salga igual en todas las páginas. — Ej.: «Gotita, una gota de agua redonda y transparente, con las mejillas rosadas. Quiere quedarse para siempre en su nube, porque le da miedo caer; pero cada vez pesa más.» — Frases: «Se llama {{protagonista}} y quiere {{deseo}}, pero {{obstaculo}}.» · «Quiere {{deseo}}, pero lo que de verdad necesita es {{necesidad}}.» · «Tiene un defecto que lo hace entrañable: {{defecto}}.» · «Por fuera es {{aspecto}}, y así sale en todas las páginas.» · «Lo acompaña {{amigo}}, que lo ayuda pero no le resuelve nada.» · «Que se parezca a la foto o al dibujo que voy a subir.»
- `escenario` · Dónde pasa · ○ — El lugar y el momento, con detalles que el lector reconozca: un cuento se siente verdadero por lo concreto —un olor, una comida, una palabra de aquí—, no por los adjetivos. — Ej.: «Una aldea de Copán en temporada de lluvias: techos de lámina, olor a tierra mojada y el río crecido.» — Frases: «En un lugar que el lector reconozca: {{lugar}}.» · «En Honduras, con cosas de aquí: la comida, las plantas y las palabras.» · «En la escuela, un día normal que deja de serlo.» · «En un mundo inventado, pero con reglas que no cambian.» · «En una casa como la del lector, hoy.»
- `concepto` · Lo que el cuento explica · ○ — Si el cuento enseña algo difícil, lo que tiene que quedar bien explicado, con sus datos exactos y la comparación que lo vuelve fácil. Si simplificas, que siga siendo verdad: un cuento que enseña algo falso lo enseña igual de bien. — Ej.: «El ciclo del agua: el sol calienta el agua y la vuelve vapor; el vapor sube, se enfría y se vuelve gotitas que forman las nubes; las gotitas se juntan, pesan y caen como lluvia. Como el vapor de la olla de los frijoles, que se vuelve gotas en la tapa.» — Frases: «Explícalo con una comparación de la vida diaria del lector.» · «Un paso del concepto en cada página, en orden.» · «Que entender el concepto sea lo que resuelve el problema del protagonista.» · «Compáralo con {{comparacion}}, y que la comparación no enseñe nada falso.» · «Usa la palabra {{termino}} y que se entienda por lo que pasa en el cuento.» · «Al final, que el lector pueda explicarlo con sus propias palabras.»
- `leccion` · Lo que se aprende · ● — Lo que el lector se lleva: una idea que entiende o un valor. Se escribe para la máquina, no para el cuento: la historia lo muestra con lo que hace el protagonista, y no lo dice al final como moraleja. — Ej.: «Que el agua que cae es la misma que subió: nada se pierde, todo da vueltas.» — Frases: «Que entienda {{idea}} sin que nadie se la explique como en una clase.» · «Que la lección se note en lo que hace el protagonista, sin moraleja al final.» · «Que equivocarse es parte de aprender.» · «Que pedir ayuda no es rendirse.» · «Que lo que da miedo se entiende mejor cuando se conoce.»
- `emociones` · Lo que debe sentir el lector · ● — Las sensaciones que se buscan y en qué orden: un cuento conmueve cuando lleva al lector de una emoción a otra, y uno que se queda en un solo tono se olvida al cerrar el libro. — Ej.: «Curiosidad al principio, un miedo que se pueda aguantar a mitad del cuento, y al final alivio y asombro.» — Frases: «Que empiece con curiosidad y termine en asombro.» · «Un miedo pequeño que se resuelve pronto, y al final calma.» · «Ternura y un poco de risa, sin tristeza.» · «Que se sienta acompañado: lo que le pasa al protagonista le podría pasar a quien lo lee.» · «Tristeza de verdad, contada con cuidado, y al final esperanza.» · «Que se ría en la página del error y se emocione al final.» · «Que conmueva sin chantaje: la emoción sale de lo que pasa, no de adjetivos que la pidan.»
- `tono` · Cómo se cuenta · ○ (sobrescribe rótulo, para qué, ejemplo y frases) — La voz del narrador y cómo suena leído en voz alta, que es como se lee un cuento (y como lo lee Storybook). — Ej.: «Como lo contaría una abuela en el corredor: cálido, sin prisa, con una frase que se repite para que los más pequeños la digan con ella.» — Frases: «Cálido y sin prisa, como un cuento antes de dormir.» · «Con humor suave y una frase que se repite en varias páginas.» · «Con rima suave, sin forzarla.» · «Contado por el propio protagonista, en primera persona.» · «Si algo es triste, se dice con cuidado y sin mentir.»
- `desenlace` · Cómo termina · ○ — La última página es la que se queda: que cierre el problema con lo que el protagonista aprendió y deje una imagen o una pregunta, no un sermón. — Ej.: «Gotita vuelve a subir convertida en vapor y, desde su nube, ve el río lleno: la última página repite la primera, pero ya sin miedo.» — Frases: «Que la última página repita la imagen de la primera, pero cambiada.» · «Un final feliz que el protagonista se haya ganado, no uno que le llegue de regalo.» · «Un final abierto que invite a imaginar qué pasa después.» · «Que termine invitando al lector a comprobarlo en su casa.» · «Que termine con una pregunta para conversar: {{pregunta}}»
- `extension` · Extensión · ● — Storybook hace libros de unas diez páginas, con una ilustración en cada una. Lo que se decide aquí es cuánto texto lleva cada página, y eso lo dice la edad. Viene RELLENO (`inicial`, en el vocabulario): «Diez páginas, una escena por página, con el texto justo para leerlo en voz alta sin cansar.» — Ej.: «Diez páginas, con dos o tres frases cortas en cada una; que cada página termine con ganas de pasar a la siguiente.» — Frases: «Diez páginas, con una o dos frases cortas en cada una.» · «Diez páginas, con dos o tres frases cortas en cada una.» · «Diez páginas, con un párrafo corto en cada una.» · «Que se lea en voz alta en unos cinco minutos.» · «Que cada página termine con ganas de pasar a la siguiente.» · «Que cada página se ate a la anterior con un «pero» o un «por eso», nunca con un «y entonces».» · «Una escena por página: el mundo del protagonista, el problema, dos intentos que fallan, lo que aprende y cómo lo resuelve.»
- `ilustracion` · Ilustraciones · ○ — Un solo estilo para todo el libro: dicho aquí, Storybook lo usa. Un dibujo o una foto de referencia no se pega aquí: se sube en Storybook, con el clip. — Ej.: «Acuarela suave, con colores cálidos y mucha luz de mañana.» — Frases: «En acuarela, con colores suaves.» · «En plastilina, como una película de animación.» · «Como un cómic, con viñetas y globos de diálogo.» · «En pixel art, como un videojuego antiguo.» · «Como un libro para colorear, en blanco y negro.» · «Tejidas a ganchillo, como muñecos de lana.» · «Inspiradas en el dibujo que voy a subir.»
- `reglas` · Reglas · ○ (sobrescribe ejemplo y frases) — (vocabulario). — Ej.: «Nada de sustos que no se resuelvan en la misma página.» / «No inventes datos; si simplificas, que siga siendo verdad.» / «Sin moraleja al final.» — Frases: «Nada de sustos que no se resuelvan en la misma página.» · «No inventes datos, fechas ni cifras: si simplificas, que siga siendo verdad.» · «Sin moraleja al final; que se entienda por lo que pasa.» · «Que sea el protagonista quien resuelva el problema; los adultos ayudan, pero no lo resuelven por él.» · «Sin marcas ni personajes de otros cuentos o películas.» · «No le mientas al lector para suavizar algo triste.» · «Si es una historia real, no inventes nada que se lea como si hubiera pasado.»
- El repaso, en este molde (§5, paso 3; todos AVISAN, ninguno para: guardar y usar se siguen pudiendo): «Protagonista» vacío, porque la máquina lo inventa y lo inventa distinto en cada página; «Para quién» sin ninguna edad (ni una cifra ni una palabra de edad: niños, adolescentes, adultos, primaria, el curso —«de sexto»—, «la tercera edad», o en inglés *toddlers*, *child*…), porque la edad manda en todo lo demás; un «Tema de la historia» que dice que el cuento explica o enseña algo con «Lo que el cuento explica» vacío, porque sin los datos exactos la máquina los inventa o los simplifica hasta que dejan de ser verdad —y solo *explicar* y *enseñar* sin negar, o un «¿Por qué…?» al principio de una frase: «una niña que aprende a andar en bicicleta» o «no entiende por qué su abuelo ya no está» son cuentos de emociones, no de datos—; «Lo que el cuento explica» de más de 120 palabras, que es mucho para un cuento breve; y, solo con Storybook, una «Extensión» que pide más de doce páginas, que Storybook no hace (el número se lee en cifra o con letras, con sus decenas —«treinta y dos», *thirty-two*— y también solo, que es lo que deja «Páginas: 20» pegado). Los números escritos con letras están en UNA tabla (`CSG_NUMEROS`), la misma del {N} de los bucles.
...
- `etiqueta` · Veredicto y etiqueta · ● (sobrescribe rótulo, para qué, inicial y frases) — Qué entrega el juez y la etiqueta de la casa. Inicial: «Al final, un veredicto de 200 palabras con la postura que gana, por qué, y las tres mejores pruebas de cada lado. Escribe el careo entero encabezado con estas líneas, una por renglón: «Título:» y un título; «Voz: A y B»; «Máquina: {{maquina}}»; «Género: entrevista»; «Consigna: {{titulo}}». Cada turno en su propio párrafo, con una línea en blanco entre uno y otro, empezando por A:, B: o JUEZ:. Termina con FIN solo en su renglón.» — Frases: «Veredicto de {{n}} palabras con quién gana y por qué.» · «Las tres mejores pruebas de cada lado.» · «Lo que ninguno probó.» · «Encabeza con Título:, Voz:, Máquina: y Género: entrevista, una por renglón.»

## 4. Las máquinas

`CSG_MAQUINAS` es una lista en el JS, nunca en la base. Cada entrada: `{ id, nombre, forma, abrir, prefill, donde, nota }`. Son las cuatro de `VOZ_MAQUINAS` más NotebookLM, Storybook y «Otra»; Copilot y Claude Code quedan fuera a propósito (nadie de la casa los nombró; el SKILL.md sale por Claude); añadirlos es una línea con `abrir` vacío.
...
| `storybook` | Storybook | `seguida`: un párrafo por bloque, con la cabecera del Cuento que enseña delante. Es el Gem de Gemini que hace el libro —unas diez páginas ilustradas y una voz que lo lee—; un dibujo o una foto de referencia se sube allí, con el clip, y para cambiar algo se le pide en el mismo chat. Es la máquina de UN molde (ver debajo). | `https://gemini.google.com/gem/storybook` | no: se abre con el texto copiado |
...
**Storybook es la máquina de UN molde.** El Cuento que enseña la trae declarada (`maquina: 'storybook'`): una pieza nueva de ese molde nace con ella, cambiar a ese molde la pone, y salir de él con la Storybook que él puso devuelve **la que había antes de entrar** (la sesión del compositor la recuerda, sin guardarla en la pieza), o la última de este aparato si la pieza se abrió ya como cuento. Una máquina elegida a mano se queda: la otra que se eligió dentro del cuento, o una Storybook elegida en un Libre, las eligió alguien. Y Storybook **no se apunta como la última**: si se apuntara, el Rápido siguiente nacería para Storybook, que no hace otra cosa que libros. Sale en todas las filas de máquinas como una más, porque un Libre pegado de otro sitio también se le puede llevar; y «Duplicar en otro molde» no se lleva a otro molde la Storybook que puso el cuento, pero sí una elegida a mano. Cuando «Pegar» propone el cuento, la nota de la propuesta dice que va para Storybook antes de abrirlo.
...
**Paso 1 — «¿Qué le vas a pedir?»** (＋ Nueva). Hoja vertical (`#csg-ver-overlay` en modo `clase`): primer renglón, 44 px y teñido, «📋 Pegar lo que ya tengo» (→ 1-bis). Debajo, cuatro baldosas en `display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 8px;` con `min-height: 88px` y `min-width: 0` (nunca un ancho fijo: 2×160 más el canal y las orillas no caben en 320 px; la sonda 17 lo mide): 💬 Prompt «un encargo de una vez» · 🧰 Habilidad «lo que sabrá hacer siempre» · 🕸 Grafo «varios que se pasan el trabajo» · 🔁 Bucle «un paso que se repite hasta que salga». Un toque abre DIRECTO el compositor con el molde recomendado. Debajo de las baldosas, otro renglón de 44 px y teñido: «🧸 Un cuento para Gemini Storybook» — «diez páginas ilustradas que emocionan y enseñan», que abre DIRECTO el compositor con el Cuento que enseña y la máquina Storybook: es un molde de Prompt, pero quien lo busca no piensa en «un prompt», piensa en un cuento, y escondido en la hoja de moldes no lo encontraría nadie.
...
6. **Bucle y lotes** (`CSG_SINONIMOS.bucle`), SOLO sobre los renglones que siguen en `tarea`/`texto` y SOLO si en el paso 4 no se reconoció NINGÚN bloque de prompt (rol, contexto, tarea, reglas, formato, ejemplos, comprobacion, tono, objetivo, estilo, audiencia, pregunta, fuentes, citas, voz, genero, tema, personaje, escenario, concepto, leccion, emociones, desenlace, extension, ilustracion): patrones → bloque: `parada`: `/^(para|parar|detente|termina)\b.*\b(cuando|que)\b/i`, `/\bhasta (que|conseguir|tener|lograr)\b/i`, `/^(until|stop when)\b/i`; `tope`: `/\b(m[aá]ximo|como mucho|no m[aá]s de|hasta)\s+\d+\s*(vueltas?|veces|intentos?|iteraci[oó]n(es)?|rondas?)\b/i`, `/\bmax(imum)?\s+\d+\s+(iterations?|attempts?|rounds?|loops?)\b/i`; `paso`: `/^(repite|rep[ií]telo|vuelve a|en cada (vuelta|iteraci[oó]n|ronda)|cada vuelta)\b/i`, `/^(repeat|each iteration|loop)\b/i`; `memoria`: `/^(guarda|lleva|conserva|recuerda)\b/i`, `/\bentre (vuelta y vuelta|vueltas)\b/i`; `lotes`: `/^(para|por) cada (uno|rengl[oó]n|elemento|tema|cap[ií]tulo|autor|caso|l[ií]nea|fila)\b/i`, `/^for each\b/i`, `/\buno por rengl[oó]n\b/i`. Con patrones de DOS clases distintas entre parada/tope/paso → Bucle · Repite hasta (cada renglón a su bloque, el resto a `paso`). Con un renglón `lotes` y una tanda de ≥ 3 renglones cortos seguidos (≤ 60 caracteres, sin punto final) → Bucle · Por lotes (el renglón → `paso`, la tanda → `lista`). Si el paso 4 SÍ reconoció bloques de prompt, no hay propuesta de bucle: la clase sigue Prompt, «Repite esto para cada tema» se queda en su bloque, y el repaso AVISA «parece que se repite (renglón 9): ¿un Bucle · Por lotes? · Duplicar en ese molde».
7. Variables detectadas; título = el `# h1` (si no es palabra de bloque) o vacío (el compositor lo propone desde Tarea).
8. Propuesta con chip «Parece un {clase} · {molde} · {n} de {m} bloques · cambiar», nunca impuesta. Molde en Prompt: con una **señal de cuento** (ver los rótulos del cuento, debajo), nada de otra forma de consigna (`rol`, `identidad`, `mision`, `objetivo`, `formato`, `voz`, `etiqueta`, `ejemplos`, `caso`, `pregunta`, `fuentes`, `citas`, `pasos`, `responde`) y dos de los cinco obligatorios del molde —una Tarea cuenta como el Tema, y la mudanza la lleva ahí— → Cuento que enseña; `objetivo`+`audiencia` → CO-STAR; `ejemplos` con ≥ 2 pares o `caso` → Con ejemplos; `pregunta`/`fuentes`/`citas` → Investigación; `voz`/`genero`/`etiqueta` → Voz prestada; `pasos`+`responde` → Razonado; solo `tarea` (y `formato`) → Rápido; nada → Libre; lo demás → Encargo completo. Habilidad: frontmatter → SKILL.md; `identidad`/`mision`/`siempre`/`nunca` → Instrucción; `disparador`+`pasos` sin `nombre` → Receta. Grafo: `coordinador`/`reparto`/`juntar` → Coordinador, si no Cadena. Bucle: `borrador`/`critico` → Borrador y crítica; `lista` → Por lotes; `postura_a` → Careo; si no, Repite hasta.
9. Nada se descarta: la sonda cuenta las palabras de entrada y las de salida (bloques + material + título) y exige que coincidan.

`CSG_SINONIMOS` (clave → id, español e inglés): rol, role, persona, eres → `rol`; contexto, context, background, situacion → `contexto`; tarea, task, instruccion, instrucciones, instruction, goal, pedido, encargo → `tarea`; objetivo, objective → `objetivo`; reglas, rules, constraints, restricciones, guidelines, no hagas → `reglas`; formato, formato de salida, format, output, output format, respuesta, response → `formato`; ejemplos, examples, few-shot → `ejemplos`; comprobacion, comprobaciones, checks, verification, validation, como se comprueba, que revisas → `comprobacion`; tono, tone → `tono`; estilo, style → `estilo`; audiencia, audience, publico → `audiencia`; ahora tu, ahora, caso, caso nuevo, now → `caso`; pasos, steps, como pensarlo, que haces → `pasos`; luego responde, responde, answer → `responde`; pregunta, question, research question → `pregunta`; fuentes, fuentes permitidas, sources → `fuentes`; citas, reglas de cita, citations → `citas`; limites, limits, scope → `limites`; voz, voice → `voz`; genero, genre, genero y largo → `genero`; tema, topic, tema o encargo → `tema`; etiqueta, etiqueta de la casa → `etiqueta`; nombre, name → `nombre`; description, descripcion, cuando se dispara, cuando, when, trigger, cuando usarla, when to use → `disparador`; recursos, resources, herramientas, tools → `recursos`; identidad, identity → `identidad`; mision, mission → `mision`; siempre, always → `siempre`; nunca, never → `nunca`; meta, goal final → `meta`; nodos, nodes, agentes, agents, especialistas → `nodos`; aristas, edges, flujo, flow, pasos y condiciones → `aristas`; estado, state, lo que viaja → `estado`; fin, fin y entrega, end, salida del grafo → `fin`; coordinador, orquestador, orchestrator → `coordinador`; reparto, como se reparte → `reparto`; juntar, como se junta → `juntar`; paso, paso que se repite, step, vuelta → `paso`; parada, criterio de parada, stop, para cuando → `parada`; tope, rondas, max, limit → `tope`; memoria, memory, acumulado, entre vueltas → `memoria`; verificacion, verify → `verificacion`; salida, que sale al final, deliverable → `salida`; borrador, draft → `borrador`; critico, critic → `critico`; revision, revise → `revision`; lista, list, elementos → `lista`; postura a → `postura_a`; postura b → `postura_b`; juez, judge → `juez`; texto, text, prompt → `texto`. Y los del Cuento que enseña, que viven aparte (`CSG_SINONIMOS_CUENTO`) porque **solo ascienden dentro de un cuento**: tema de la historia, de que trata, trama, plot, story idea → `tema`; para quien, edad del lector, publico objetivo, target age, target audience → `audiencia`; protagonista, protagonistas, personaje, personajes, protagonist, protagonists, character, characters, main character → `personaje`; escenario, donde pasa, ambientacion, setting → `escenario`; concepto, lo que el cuento explica, que explica → `concepto`; leccion, lo que se aprende, aprendizaje, moraleja, ensenanza, lesson, moral, moral of the story → `leccion`; emociones, emocion, lo que debe sentir el lector, lo que debe sentir, sensaciones, sentimientos, sentimiento, emotions, feelings → `emociones`; como se cuenta, narrador, voz narrativa, narrator → `tono`; desenlace, como termina, final del cuento, ending → `desenlace`; extension, paginas, numero de paginas, pages, number of pages → `extension`; ilustracion, ilustraciones, estilo de ilustracion, estilo de las ilustraciones, estilo visual, estilo artistico, dibujos, art style, illustrations, illustration style → `ilustracion`. «Final» a secas NO entra: es el «Fin» de un grafo o de un bucle dicho de otra manera, y ascenderlo partiría esas consignas en un bloque que no es suyo (la asimetría de la regla 8). Ni «edad» ni «concept», tampoco dentro de un cuento: «Edad: 6 años» puede ser la del lector o la del protagonista, y *concept* la idea de la historia o lo que explica; colocarlos sería adivinar, y se quedan donde están, nombrados.

**⚠️ Los rótulos del cuento solo ascienden dentro de un cuento** (añadido el 26 de septiembre de 2026, por la revisión, antes de publicarlo). Mezclados con los demás, «Edad: 3 años» en un prompt a un pediatra salía de su Contexto para ir a «Audiencia», «Concepto: inflación» en un glosario se rebautizaba «Lo que el cuento explica», «Setting: B2C» se volvía «Dónde pasa», y un juego de rol con «Personaje:» y «Escenario:» se proponía como cuento para Storybook; la cuenta de palabras decía «no se perdió ninguna» porque la palabra de la persona iba a la estructura. Por eso `csgLeer` lee en **dos vueltas**: la primera sin ninguno de estos rótulos; si ve una **señal** —un rótulo que solo escribe quien pide una historia (tema de la historia, story idea, moraleja, moral of the story, desenlace, final del cuento, lo que el cuento explica: `CSG_LEC_SENALES_CUENTO`) o la cabecera del Cuento que enseña— y nada que lo vete, lee otra vez con todos, y esa segunda lectura solo se queda **si propone el Cuento que enseña**. Así, **un texto que no se propone como cuento se lee exactamente como se leía antes de que el cuento existiera**. Y fuera de un cuento no se sugiere ninguno («¿Querías Dónde pasa?»). Los bloques que solo tiene el molde del cuento (`CSG_LEC_IDS_CUENTO`, sacados de los moldes) **no votan la clase**: hablan de la historia, no de la forma de la consigna, y con voto unas instrucciones de Gem que cuenta cuentos se proponían como prompt.

**Y la mudanza del lector no toma de `CSG_EQUIVALE` lo que no es la misma cosa con otro nombre** (`CSG_LEC_NO_MUDA`): en cualquier molde, «Nunca:» y «Siempre:» no van a Reglas —la palabra del rótulo ES la regla, y «Nunca: uses jerga técnica» se quedaba en «uses jerga técnica»—; y en el cuento, «Género: fábula» no pisa la Extensión ni «Estilo:» va a «Cómo se cuenta» (puede ser el de los dibujos). Lo que no se muda se queda aparte con su rótulo; lo de «Nunca:» es la única diferencia fuera del cuento, y es el arreglo de un fallo de antes. Lo que sí añade (`CSG_LEC_MUDA`), y solo en el cuento: lo que se pide ES su tema (el cuento no tiene Tarea), y el aviso nombra «lo que venía sin rótulo» sin llamarlo «Tarea», que nadie escribió.

**Paso 2 — el compositor.** Cabecera compacta: ‹ · el título (campo con marca de agua «Título»; se propone con las primeras siete palabras de Tarea/Pregunta/Nombre/Meta/Paso/Borrador mientras la persona no lo toque, `_csgAuto`) · [👁] vista previa. Primera fila, deslizable: chip «Molde: Rápido ▾» (hoja vertical con los moldes de la clase, 44 px cada uno, nombre + «cuándo», el recomendado primero; cambiar de molde conserva los bloques con el mismo id y dice cuáles quedan como bloques libres), y dentro de esa hoja el renglón «❓ ¿Cuál elijo?» con cuatro preguntas de sí/no (¿es un cuento, para emocionar o para enseñar algo? → Cuento que enseña; ¿tienes ejemplos de cómo quieres la salida? → Con ejemplos; ¿tiene que calcular o decidir entre opciones? → Razonado; ¿se va a quedar puesta en un Proyecto o Gem? → Instrucción de sistema; nada → Encargo completo); después los chips de máquina con la última usada marcada; después los chips de estante del autor y «＋ estante» (cada estante ≤ 40 caracteres, máximo 12 por pieza: el JSON tiene tope y el botón lo dice al llegar).

Debajo, los BLOQUES apilados, cada uno una tarjeta: rótulo con su punto (● / ○), el «para qué» en una línea gris (se pliegan todos con el interruptor de aparato «Sin explicaciones»), un recuadro que crece (`csgCrecer`, como `rcuCrecer`) con letra de 16 px (a menos de 16 el iPad hace zoom al enfocar), el EJEMPLO como placeholder (literal del JS) y, mientras el recuadro está vacío, un enlace «📄 usar el ejemplo» que lo copia para editarlo (con texto escrito no sale: el ejemplo se ve mientras hace falta y no pisa nada). Los bloques con `inicial` nacen con ese texto puesto (la etiqueta de Voz prestada y Careo, el tope de Por lotes). DEBAJO del recuadro, la fila de FRASES HECHAS: chips deslizables de 40 px como mínimo, con la frase en dos renglones como mucho (hay frases de 85 caracteres: en uno solo el chip saldría más ancho que la pantalla; entera en su `title` y en el recuadro en cuanto se toca), y al final «{ } Variable», que mete `{{ }}` con el cursor dentro. Los obligatorios ABIERTOS; los opcionales PLEGADOS a un renglón «○ Rol · ＋». Al enfocar, `scrollIntoView({block:'center'})`; el foco va dentro del mismo toque, sin `await`.
...
**Paso 5 — Guardar.** Siempre se puede: una pieza a medias es un borrador, no un error. Local al instante (`faro_consigna_v1`), nube con respiro de dos segundos (`csgSubirLuego`). Si la versión de AHORA ya se usó (algún uso de la bitácora lleva su `v`) y el texto armado en `md` cambió, nace la versión n+1 y la n queda en `versiones`; mientras la n+1 no se use, los retoques la corrigen en su sitio (un 💾 por retoque haría nacer diez versiones sin usar y plegaría el texto de la que tenía el 👍). Se escribe SOLO lo que la persona cambió en el compositor desde que lo abrió (título, forma, máquina, estantes a tres bandas, material): un 💾 sin cambios no escribe nada, y lo que otro aparato —o el menú ⋯— cambió mientras tanto no se deshace; si los dos cambiaron el texto, gana el de aquí y el de allá queda como versión, y se dice. Si el título normalizado ya existe en otra pieza viva: «Ya hay una «Resumen de informe» · abrirla · guardar igual»; «abrirla» no tira lo escrito: si dice otra cosa, se guarda aparte antes de abrir la otra (y el renglón lo dice), y si dice lo mismo no hace falta guardarla. Con la franja «tenías cambios sin guardar» puesta, la primera tecla guarda aparte lo de antes («… (lo que no se guardó)») y lo dice: ninguna de las dos versiones se pierde sin decidirlo.
...
2. **ELEGIR O PEGAR.** Una baldosa abre el molde recomendado; «🧸 Un cuento para Gemini Storybook» abre el Cuento que enseña con Storybook puesta; «📋 Pegar lo que ya tengo» reparte y PROPONE clase y molde. La herramienta sola: bloques, variables, material y clase; nombra lo que no entendió. Se pregunta: nada, salvo confirmar con un chip.
...
7. **RENDIR CUENTA, DE UN TOQUE Y DESPUÉS.** La bitácora se PREGUNTA (robado de El Pliego biblioteca: el «¿Cómo salió?» inmediato preguntaba antes de que la máquina contestara, y una bitácora que hay que acordarse de escribir no la escribe nadie). En la siguiente apertura del anaquel, el renglón «Usaste «Resumen de informe» en Claude hace 2 h · ¿sirvió? ✅ 〰 ❌ ✕» pregunta por UN uso, el más reciente, solo de los últimos 7 días. Un toque guarda `ok` (`'si'|'regular'|'no'`) y abre, sin obligar, un campo de una línea «qué pasó» (`maxlength=300`). ✕ deja ese uso sin contestar para siempre y pasa al siguiente. Ignorado en tres aperturas seguidas (`faro_consigna_pregunta_v1`), se pliega a «3 usos sin contestar ▸». Una apertura es entrar al anaquel desde fuera de la herramienta: volver con ‹ del compositor no cuenta, y solo se pregunta por los usos de ANTES de abrir (el que se apunta ahora espera a la siguiente). Si en la misma apertura hay un compositor a medio escribir, «seguir» se queda el hueco la primera vez que se ofrece ese borrador, y después se lo cede a la pregunta. Nunca se inventa un resultado.
...
5. La forma la decide `molde.forma` si existe (`skill`, `seguida`); si no, `CSG_MAQUINAS[maquina].forma`.
   ...
   - **`seguida`**: un párrafo por bloque, `{Rótulo}: {frase}`, separados por línea en blanco; `lista` y `ejemplos` → cada renglón sin viñeta («- », «• » fuera; los números se quedan), con «.» al final si no termina en `.!?:…`, unidos por un espacio (los pares se escriben «a → b.»); `rotuloMayus` pone el rótulo en mayúsculas. Material como en `md`.
   - **`cabecera`** (del molde) va delante de todo con una línea en blanco, **en las tres formas**: en `xml`, pelada encima de las etiquetas y dentro del `encargo`. En el Cuento que enseña es lo único que pide algo (su molde no tiene Tarea), y es además la firma con que el lector reconoce lo que salió de aquí.
...
**Cuento que enseña en `seguida` (Storybook)** (la cabecera del molde delante; cada bloque en su párrafo, las Reglas como frases seguidas; con Claude, el mismo cuento sale en `xml`, y con ChatGPT o Gemini, en `md`, las dos con la misma cabecera delante):
```
Crea un cuento ilustrado con estas indicaciones.

Tema de la historia: Un cuento que explique por qué llueve, contado por una gota de agua que tiene miedo de caer.

Para quién: Niños de 6 a 8 años que empiezan a leer solos; muchas veces se lo leerá un adulto en voz alta.

Protagonista: Gotita, una gota de agua redonda y transparente, con las mejillas rosadas. Quiere quedarse para siempre en su nube, porque le da miedo caer; pero cada vez pesa más.

Dónde pasa: Una aldea de Copán en temporada de lluvias: techos de lámina, olor a tierra mojada y el río crecido.

Lo que el cuento explica: El ciclo del agua: el sol calienta el agua y la vuelve vapor; el vapor sube, se enfría y se vuelve gotitas que forman las nubes; las gotitas se juntan, pesan y caen como lluvia. Como el vapor de la olla de los frijoles, que se vuelve gotas en la tapa.

Lo que se aprende: Que el agua que cae es la misma que subió: nada se pierde, todo da vueltas.

Lo que debe sentir el lector: Curiosidad al principio, un miedo que se pueda aguantar a mitad del cuento, y al final alivio y asombro.

Cómo se cuenta: Cálido y sin prisa, como un cuento antes de dormir.

Cómo termina: Que la última página repita la imagen de la primera, pero cambiada.

Extensión: Diez páginas, con dos o tres frases cortas en cada una.

Ilustraciones: En acuarela, con colores suaves.

Reglas: Nada de sustos que no se resuelvan en la misma página. No inventes datos, fechas ni cifras: si simplificas, que siga siendo verdad. Sin moraleja al final; que se entienda por lo que pasa.
```
...
Al final, un veredicto de 200 palabras con la postura que gana, por qué, y las tres mejores pruebas de cada lado. Escribe el careo entero encabezado con estas líneas, una por renglón: «Título:» y un título; «Voz: A y B»; «Máquina: ChatGPT»; «Género: entrevista»; «Consigna: Careo jornada extendida». Cada turno en su propio párrafo, con una línea en blanco entre uno y otro, empezando por A:, B: o JUEZ:. Termina con FIN solo en su renglón.
...
10. Fusión: unión por `uid`/`vid`; `ok` gana a `null`; escalares por `actualizado`. Con el compositor abierto y la nube trayendo la corrección de otro aparato: 💾 sin tocar nada no escribe nada; cambiar solo el título escribe solo el título; si los dos cambian el texto, gana el de aquí y el de allá queda en Versiones. Tres 💾 sobre una pieza usada hacen nacer UNA versión.
...
12. Usar: ▶ → hoja medida con `elementFromPoint` sobre los tres botones del pie a 390 y 320 px (y, a 320×568, que «Intro» de variable en variable deje cada recuadro ENCIMA del pie pegado); Copiar apunta `usos`, `ultima` y un uso con `ok: null`; ↗ Abrir: portapapeles ANTES de `w.open`, dirección = literal + `encodeURIComponent(armado)`, `noopener,noreferrer`; con 1.501 codificados se abre pelada y el botón lo dice; Gemini pelada siempre.
...
15. Borrador: sin Tarea se guarda, la ficha dice «🟡 borrador: falta Tarea» en su única línea corrida, USAR se para; bucle sin tope igual; título repetido avisa con enlace, y «abrir la que ya hay» no tira lo escrito. Con la franja «tenías cambios sin guardar» puesta, ni abrir otra pieza ni escribir encima pierden lo de antes.
...
23. ☑ Elegir varias (23 de septiembre de 2026): la describe la regla 35 de la normativa de `CLAUDE.md`, con sus nueve apartados.
24. 🧸 El cuento para Gemini Storybook (26 de septiembre de 2026), ordenado por gravedad y PULSADO: ↗ Abrir copia ANTES de abrir, abre `https://gemini.google.com/gem/storybook` pelado y con `noopener,noreferrer`, lo copiado es lo que se ve con la cabecera delante, y el aviso nombra adónde ir; usar Storybook NO la apunta como la última, y un Rápido nuevo nace para la de antes; el renglón de ＋ Nueva a 320 px, de 44 px como mínimo, dentro de la hoja, debajo de las baldosas y sin nada encima (`elementFromPoint`); un toque abre el Cuento que enseña con Storybook y «Tema de la historia» enfocado DENTRO del toque, los cinco obligatorios abiertos y los siete del oficio plegados; la Extensión nace rellena; con los obligatorios no para nada y avisa del protagonista y de lo que explica; lleno, ni para ni avisa; la vista previa es carácter por carácter `csgArmar`, y el chip de Storybook se ve ENTERO en la vista previa y en Usar a 320 px; «¿Cuál elijo?» dice «cuatro preguntas», la primera es la del cuento, y cambiar de molde pone Storybook y la devuelve, diciéndolo —y con una máquina elegida a mano que no es la última del aparato, la ida y vuelta por el cuento vuelve a ELLA, y un Libre con Storybook elegida a mano la conserva al pasar a Rápido—; y el cuento del §7 pegado se propone como «Cuento que enseña · 12 de 12 bloques», la nota dice que va para Storybook antes de abrirlo, y vuelve a armarse EXACTAMENTE igual.
...
- No hace el libro: el libro lo hace Storybook, y La Consigna le escribe el encargo. No hay ninguna puerta para que lo haga desde aquí (Storybook no tiene API ni recibe el texto en la dirección), así que se copia y se pega; la foto o el dibujo de referencia se sube allí, con el clip, y lo que haya que cambiar se le pide en el mismo chat.
