...
**2026-08-29** · 28 fuentes, 78 direcciones candidatas probadas.
...
| ⁉️ Dudosas | **1** | Responden con ítems, pero sin fecha, sin DOI y sin resumen: casi seguro **no son artículos** |
| ⚠️ Sin canal | **5** | La institución está en pie y no publica canal legible |
...
| Nada es Gratis (FEDEA) | C | **❌ no responde** | · | · | · | · | · |
...
| COHEP | C·HN | **🚫 rechaza recolectores** | · | · | · | · | · |
| FOSDEH | C·HN | **❌ no responde** | · | · | · | · | · |
| Bolsa Centroamericana de Valores | C·HN | `rss` | 10 | ✅ | · | en⚠es | 1.1/día |
...
- **Nada es Gratis (FEDEA)** — no responde
  - `https://nadaesgratis.es/feed` → ⚠️ fetch failed
  - `https://nadaesgratis.es/feed/atom` → ⚠️ fetch failed
  - `https://nadaesgratis.es/rss` → ⚠️ fetch failed
- **SciELO** — rechaza
...
- **COHEP** — rechaza
  - `https://cohep.com/?feed=rss2` → 403 · json · 0 ítems
  - `https://cohep.com/feed/` → 403 · json · 0 ítems
  - `https://cohep.com/` → 403 · json · 0 ítems
- **FOSDEH** — no responde
...
- **Crossref** — dudoso
  - `https://api.crossref.org/works?rows=5&sort=published&order=desc` → 200 · json · 5 ítems
