...
**4. Sueltos que no bloquean nada pero siguen ahí:**
   ...
   · 🗑 **RETIRADA** — hubo un rato una Edge Function `voz-actividades-ia`
     que le pedía a Claude las actividades de comprensión, con su clave de
     Anthropic en los Secrets y su despliegue a mano desde el panel. **Ya no
     hay nada que desplegar ni ninguna clave que comprar**: el autor lo
     precisó el 16 de septiembre de 2026 —«no quiero APIs ni nada de eso; el
     sistema debe tener las actividades una vez que se pegue o suba un
     ensayo»— y tenía razón. Las actividades las monta ahora el aparato, del
     propio texto y en el momento de guardarlo. Si quedó desplegada en el
     panel de alguna prueba, se puede borrar; y el secreto `ANTHROPIC_API_KEY`,
     si llegó a ponerse, ya no lo usa nadie. El porqué, largo, está en la
   ...
   · ✅ **CORRIDO** — `supabase/sql/metas_videos_tope_10.sql`, el 28 de
     ...
     preguntas por video, que el autor pidió ese mismo día. Es lo mismo
     que volver a pegar `metas_videos.sql` entero —el tope también subió
     allí—, pero son treinta líneas en vez de trescientas.
     ⚠️ **Si el tope vuelve a cambiar, se cambia en TRES sitios a la
     vez**: este `check`, `MVID_MAX_PREG` en `js/tools/metas-videos.js` y
     el tope de `vmPreguntas` en M.E.T.A.S. El que se quede corto no da
     ningún error: pierde preguntas por el camino. Y el `check` se tira
     y se vuelve a poner a propósito —con un «si no existe», re-correrlo
     diría «Success» y dejaría el número viejo—, así que lo que hay que
     mirar al correrlo es el `tope_preguntas` del final, nunca el
     «Success» del editor.
